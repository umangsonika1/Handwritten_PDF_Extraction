...
def generate_html(sorted_line_boxes, output_file, img, im_np, c):
    if c==0:
...
def process_image(uploaded_file, img_pdf):
    if img_pdf==0:
    ...
    if uploaded_file.type == "application/pdf":
        ...
        if num_pages > 0:
            for page_number in range(num_pages):
                ...
                im_np_f, elements, latexlst, bounding_box, img_cat = process_image(img_bytes, 1)
                bbox_lst = []
                for idx, (class_id, x1, y1, x2, y2) in enumerate(bounding_box, start=1):
                    bbox_lst.append((class_id, x1, y1, x2, y2))

                output_path = r"C:\Users\Brigosha_Guest\PycharmProjects\pythonProject1\output\newpdf2_page{}.html".format(
                    page_number)

                # generate_html(bbox_lst, output_path, im_np_f, im_np_f, 1)

                generate_html(bbox_lst, output_path, im_np_f, im_np_f, 0)
            ...
            output_folder = r"C:\Users\Brigosha_Guest\PycharmProjects\pythonProject1\output"
            for page_number in range(0, num_pages):  # Assuming num_pages is the total number of pages
                html_file_path = os.path.join(output_folder, f"newpdf2_page{page_number}.html")

                with open(html_file_path, "rb") as file:
                    file_contents = file.read()

                file_name = f"newpdf2_page{page_number}.html"
                b64 = base64.b64encode(file_contents).decode()
                href = f'<a href="data:file/html;base64,{b64}" download="{file_name}">Download File {page_number}</a>'
                download_link = href
        ...
        im_np_f, elements, latexlst, bounding_box, img_cat = process_image(uploaded_file, 0)

        bbox_lst = []
        for idx, (class_id, x1, y1, x2, y2) in enumerate(bounding_box, start=1):
            bbox_lst.append((class_id, x1, y1, x2, y2))

        # generate_html(bbox_lst, r"C:\Users\Brigosha_Guest\PycharmProjects\pythonProject1\output\newf2.html" ,im_np_f, im_np_f, 1)
        generate_html(bbox_lst, r"C:\Users\Brigosha_Guest\PycharmProjects\pythonProject1\output\newf2.html" ,im_np_f, im_np_f, 0)
